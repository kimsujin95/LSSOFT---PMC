      ...
      <div class="row">
        <div class="col-lg-8 col-md-10 mx-auto">
          <div class="page-heading">
            <h1>Map Reserve</h1>
            <span class="subheading">지도를 통해 예약해보세요</span>
          </div>
        </div>
      </div>
    </div>
...
<head>
    ...
    .overlay_info .carinfo {font-size: 12px; color: #333; position: absolute; left: 90px; right: 14px; top: 24px; white-space: normal}
...
<body>
<div id="centerDiv">
<h1 id="content">주소를 확인해 보세요</h1><br>
<p id="contenttext">버튼을 누르면 예약가능한 장소가 표시되며<br> 
...
<script>
   ...
   $(document).ready(function(){
      $.getJSON('/resources/sungnamparking.json', function(data){
...
function showMarkers(){ //마커를 보이게하는 함수
   ...
   for (var i = 0; i < 159; i++){
      ...
      position = {
            ...
            '   <a href="/car/carlist?nowlocation=' + i + '" target="_blank" class="link"><strong>' +   parkingPositions[i].title + '</strong></a>' + 
            '   <div class="desc">' + 
            '       <img src="/resources/parkImg/park_' + i + '.jpg" alt="">' +
            '       <span class="address">' + parkingPositions[i].address + '</span>' +
/* 			'		<span class="carinfo"><c:forEach items="${list}" var="carinfo">' +
			'								<tr><td><c:out value="${carinfo.carmodel}"></c:out></td>' +
			'							</tr></c:forEach>' + i + '</span>' +  */			
            '   </div>' +
            '</div>'
      };
